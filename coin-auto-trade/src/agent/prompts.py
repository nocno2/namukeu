...
3. 추세 추종: 강한 상승 모멘텀을 가진 종목을 발굴하여 수익을 길게 가져가라.
4. 소액 계좌 특성 반영: 잃어도 다시 시작할 수 있는 규모임을 인지하고, 안정성보다는 높은 기대 수익률에 배팅하라.
...
2. 기대 수익비(Risk/Reward)가 1:2 이상이라면 적극적으로 승인하라.
3. 전략가가 강력한 확신(Confidence 0.8 이상)을 보인다면, 리스크가 있더라도 비중 확대를 허용하라.
4. 손절가는 엄격히 지키되, 진입 자체는 유연하게 승인하라.
