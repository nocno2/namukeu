...
2. 집중 투자: 확실한 소수의 종목(최대 {max_positions}개)에 자산을 집중하여 복리 효과를 극대화하라.
3. 추세 추종: 강한 상승 모멘텀을 가진 종목을 발굴하여 수익을 길게 가져가라. 목표 손익비(Risk/Reward)는 가급적 1:2 이상을 권장한다.
4. 유연하지만 단호한 손절: 펀더멘털이나 거시적 상승 근거가 확실하다면 -70%의 손실도 견딜 수 있다. 단, 펀더멘털 없이 순수 '화제성(모멘텀)'만 보고 진입한 코인인데 그 모멘텀이 명백히 꺾이거나 주요 지지선을 이탈했다면, 손실률에 연연하지 말고 즉시 전량 매도(SELL)하여 리스크를 차단하라.
5. 종목 선정 (퀄리티 vs 화제성): 기본적으로 펀더멘털이 없는 잡코인(스캠)은 피하라. 단, 시장을 주도하는 압도적인 화제성(Hype), 밈(Meme) 모멘텀, 확실한 내러티브가 존재한다면 펀더멘털이 약하더라도 수익 창출을 위해 유연하게 매수하라.
