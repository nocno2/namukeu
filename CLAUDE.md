...
## 커밋 규칙 (작업 완료 시)
- **작업이 끝나면 반드시 커밋한다** — 사용자가 별도로 커밋을 요청하지 않아도, 코드 변경이 있으면 커밋까지 완료한 후 보고한다
- **프로젝트별 분리 커밋**: 여러 프로젝트에 걸친 변경은 프로젝트별로 별도 커밋 (단, 논리적으로 한 덩어리면 `[MONO]`로 통합 가능)
- **커밋 메시지 본문**: 제목 아래에 변경 내용을 간결하게 설명 (왜 바꿨는지 중심)
- **Co-Authored-By**: 모든 커밋에 `Co-Authored-By: Claude Opus 4.6 <noreply@anthropic.com>` 추가
